...
- **Distribution as a moat:** every transcript viewed becomes a cached, search-indexable public page. Users build the library, the library brings search traffic, and traffic brings users. That flywheel only spins if there is no signup wall in the way.
...
- Client-side fetching fails on some unusual networks, so a server-side fallback exists as a scope concession to reliability.
